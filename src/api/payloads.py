...
from src.metrics.indicators import compute_indicator_series
...
from src.metrics.samples import ceil_div, filter_samples_since
...
def tick_display_verdict(
    monitor: PingMonitor,
    stabilizer: VerdictStabilizer,
    *,
    now: datetime | None = None,
) -> dict:
    """Advance stabilizer dwell timers without rebuilding the full live payload."""
    trend_samples = monitor.get_samples(trend_window_minutes())
    now_samples = filter_samples(trend_samples, now_window_minutes())
    now_stats = compute_now_stats(now_samples, now=now)
    flow = compute_baseline_and_spikes(now_samples, now=now)
    instant = compute_instant_verdict(now_stats, flow)
    return stabilizer.update(instant["level"], now=now)
...
def build_metrics_payload(
    ...
    latency_distribution = compute_latency_distribution(samples)
    now_payload, recent_samples, indicator_series = _now_and_series(
        monitor, stabilizer, trend_samples
    )
    latest = monitor.get_latest_sample()
