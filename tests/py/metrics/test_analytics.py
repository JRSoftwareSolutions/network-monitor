...
def test_downsample_samples_averages_jitter_per_bucket():
    end = datetime(2026, 6, 14, 12, 0, 0, tzinfo=timezone.utc)
    samples = [
        sample(end, -3, latency_ms=10, jitter_ms=2),
        sample(end, -2, latency_ms=20, jitter_ms=4),
        sample(end, -1, latency_ms=30, jitter_ms=6),
        sample(end, 0, latency_ms=40, jitter_ms=8),
    ]
    result = downsample_samples(samples, max_points=2)
    merged = next(point for point in result if point["jitter_ms"] == 5.0)
    assert merged["latency_ms"] == 25.0


def test_compute_latency_distribution_counts_all_samples():
    now = datetime(2026, 6, 14, 12, 0, 0, tzinfo=timezone.utc)
    samples = [
        sample(now, 0, latency_ms=30),
        sample(now, 1, latency_ms=60),
        sample(now, 2, latency_ms=100),
        sample(now, 3, latency_ms=150),
        sample(now, 4, success=False),
    ]
    dist = compute_latency_distribution(samples)
    assert dist == {"great": 1, "good": 1, "okay": 1, "bad": 1, "failed": 1}
