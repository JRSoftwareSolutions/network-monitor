<script lang="ts">
  ...
  onMount(() => {
    ...
    chart = createLatencyChart(container, times, min, max, avg, bucketSeconds, bounds);
  ...
  $effect(() => {
    ...
    const { times, max, min, avg } = buildLatencySeries(untrack(() => buckets));
    ...
    if (
      latencySeriesEqual({ times: lastTimes, avg: lastAvg }, { times, avg }) &&
    ...
    updateLatencyChart(chart, times, min, max, avg, bs, bounds);
    lastAppliedRevision = revision;
    lastTimes = times;
    lastAvg = avg;
    lastBounds = bounds;
